...
public class BookDAO {
	...
	public int insertBook(Book book) throws SQLException, ClassNotFoundException {
		...
		int result = 0;
		String insert = "INSERT INTO Book (name, category_id, amount, image, location, tacgia, nxb) VALUES (?,?, ?, ?, ?, ?, ?)";
		...
		preSt.setString(5, book.getLocation());
		preSt.setString(6, book.getTacgia());
		preSt.setString(7, book.getNxb());
...
//		String sql = "Select * from Book where name like '%l';";
	...
	public int updateBook(Book book) throws SQLException, ClassNotFoundException {
		...
		if (conn == null)
			conn = ConnectDatabase.getMySQLConnection();
		String sql = "Update Book set name =?,category_id =?,amount =?,image =? ,location =?, tacgia = ?, nxb = ? where id=? ";
		PreparedStatement pstm = (PreparedStatement) conn.prepareStatement(sql);
		...
		pstm.setString(5, book.getLocation());
		pstm.setString(6, book.getTacgia());
		pstm.setString(7, book.getNxb());
		pstm.setString(8, book.getId());
